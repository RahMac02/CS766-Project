...
if __name__ == "__main__":
    ...
    # add variables to track computation time and error
    avg_time = np.array([])
    err = 0
    while dataset.isOk() and img_id<840:
        ...
        if img is not None:
            ...
            avg_time = np.append(avg_time,time.time() - start_time) # update computation time
    ...
    if is_draw_err:
        err_plt.quit()
    # printing average computation time & error
    print("Avg Time: ", np.average(avg_time))
